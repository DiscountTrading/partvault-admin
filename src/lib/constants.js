export const APP_VERSION = '3.36.83'
...
// Whether this store applies an estimated cost base to its parts. NEW stores are
// created with `costing.enabled = false` (see store-creation) → they show pure
// eBay revenue with $0 cost until the seller opts in from Settings → Costs.
// EXISTING stores have no `enabled` key → treated as ON, so nothing changes for
// anyone already using PartVault. Manually-entered per-part costs always count.
export const costsEnabled = (costing = {}) => costing?.enabled !== false
...
export const DEFAULT_BASE_COST_PCT = 25 // fallback part cost as % of sale price (when the cost base is on)
...
export const partEffectiveCost = (p = {}, costing = {}) => {
  ...
  // When the cost base is off (new stores by default), no estimate is applied —
  // the part costs only what's actually recorded on it. Manually-entered costs
  // (in `recorded`) always count either way.
  const supplement = costsEnabled(costing)
    ? b.baseCost + b.labour + b.admin + (manualPost > 0 ? 0 : b.postage) + sc.value
    : 0
